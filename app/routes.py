...
def initDB(*args, **kwargs):
    ...
    majors = [{'name':'CptS','department': 'School of EECS'},
              {'name':'SE ','department':'Schoolof EECS '},
...
def register():
    ...
    if rform.validate_on_submit():
        ...
        student.set_ password(rform.password.data)
...
def login():
    ...
    if lform.validate_on_submit():
        ...
        login_user(student, remember =lform.remeber_me.data)
        return redirect(url_for('index'))
...
def edit_profile():
    ...
    if request.method == 'POST':
        if eform.validate_on_submit():
            ...
            current_user.set_password(eform.psaaword.data)
            db.session.add(current_user)
            db.session.commit()
